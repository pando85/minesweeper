...
fn pick_random<T>(rnd: &mut impl RngCore, v: &mut Vec<T>) -> Option<(T, Vec<T>)>
where
    T: Clone,
{
    if v.is_empty() {
...
fn get_mines(mines_n: usize, len: usize) -> Result<Vec<bool>, MinesOverflowError> {
    ...
    for _ in 0..mines_n {
        ...
        let (mine_index, new_indexes) = pick_random(&mut rng, &mut indexes).unwrap();
        indexes = new_indexes;
...
pub fn get_grid(
    ...
    for _ in 0..max_threads {
        ...
        'thread_work: for _ in 0..cols_per_thread {
            let (x, remaining_cols) = match pick_random(&mut rng, &mut cols) {
...
mod tests {
    ...
    fn test_pick_random() {
        let mut rng = SmallRng::from_entropy();
        let (value, v) = pick_random(&mut rng, &mut vec![1, 2, 3]).unwrap();
